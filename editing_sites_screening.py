...
dna_orf_alignment = pysam.AlignmentFile('octo_dna_orf_alignmen_sorted.bam','rb')
...
orf_fasta = FastaFile('swissprotORF.fasta')
...
"""
# Testing search_refbase function with SeqIO from Biopython,
...
def strong_editing_site_detection(dna_orf_alignment, ses, ubs):
    ...
    for i in mapped_ref_seq:
        ...
        if counter % round(total_seq/1000) == 0:
            print("ses progress: %.1f%s" % ((counter/total_seq), '%'))
...
def weak_editing_site_detection(rna_orf_alignment, q, sample_id):
    ...
    for i in ubs:
        ...
        if counter % round(total/1000) == 0:
            print("wes sample %d progress: %.1f%s" % (sample_id, (counter/total), '%'))

    wes_pval = [i[-1] for i in wes ]
    multitest_results = multipletests(wes_pval,alpha=0.1,method='fdr_bh')

    for i, j in zip(multitest_results[1], range(wes)):
        wes[j][-1] = i
...
def count_bases(rna_orf_alignment, es, q, sample_id):
    ...
    for i in es:
        ...
        counter += 1
        if counter % round(total/1000) == 0:
            print("sample %d count bases progress: %.1f%s" % (sample_id, (counter/total), '%'))
...
def add_edit_info(es):
    ...
    for i in range(len(es)):
        ...
        es[i].append(genomic_codon)
        es[i].append(edited_codon)
        es[i].append(codon_dict[genomic_codon])
        es[i].append(codon_dict[edited_codon])
        es[i].append(edit_codon_pos)
        es[i].append(upstream_base)
        es[i].append(downstream_base)
...
if __name__ == '__main__':
    ...
    wes_process = []
    for x, y, z in zip(rna_orf_alignment_list,wes_queue_list,range(1,7)):
    ...
    wes_all_id_loc = (wes_all_id_loc[0] | wes_all_id_loc[1] |
                      wes_all_id_loc[2] | wes_all_id_loc[3] |
                      wes_all_id_loc[4] | wes_all_id_loc[5])
    ...
    for x,y,z in zip(rna_orf_alignment_list,wes_countbases_queue,range(1,7)):
    ...
    for x,y,z in zip(rna_orf_alignment_list,ses_countbases_queue,range(1,7)):

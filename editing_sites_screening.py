...
dna_orf_alignment = pysam.AlignmentFile('octo_dna_orf_alignment_mapq30_sorted.bam','rb')

octo_bam_file_name = ['octo1_rna_orf_alignment_mapq30_sorted.bam',
                      'octo2_rna_orf_alignment_mapq30_sorted.bam',
                      'octo3_rna_orf_alignment_mapq30_sorted.bam',
                      'octo4_rna_orf_alignment_mapq30_sorted.bam',
                      'octo5_rna_orf_alignment_mapq30_sorted.bam',
                      'octo6_rna_orf_alignment_mapq30_sorted.bam']
...
def strong_editing_site_detection(dna_orf_alignment, ses, ubs):
    ...
    for i in mapped_ref_seq:
        ...
        counter += 1
        if counter % round(total_seq/100) == 0:
            print("ses progress: %d%s" % (round((counter/total_seq)*100), '%'))
...
def count_bases(rna_orf_alignment, es, q, sample_id):
    ...
    for i in es:
        ...
        counter += 1
        if counter % round(total/100) == 0:
            print("sample %d count bases progress: %d%s" % (sample_id, round((counter/total)*100), '%'))
    q.put(None)
    q.close()
...
if __name__ == '__main__':
    ...
    for i in wes_queue_list:
        ...
        while True:
            a = i.get()
            print(a)
            if a == None: break # None in q indicates last item
            temp.append(a)
        wes_all.append(temp)
